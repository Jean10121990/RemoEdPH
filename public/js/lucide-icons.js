/**
 * Lucide system icons for RemoEd UI. Catalog: https://lucide.dev/icons/
 * Paths copied from lucide (ISC). Do not replace with emoji.
 */
(function (global) {
  'use strict';
  ...
  function applyToggle(el, onName, offName, isOff) {
    if (!el) return;
    apply(el, isOff ? offName : onName);
  }

  function fillAll(root) {
    var scope = root && root.querySelectorAll ? root : document;
    scope.querySelectorAll('[data-lucide]').forEach(function (node) {
      apply(node, node.getAttribute('data-lucide'));
    });
  }
