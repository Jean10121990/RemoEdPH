r"""Build L4M1 Young Stewards (Month 1 - The Map of My Heart) Lessons 1-3 as RemoEd PPTX decks.

Engine copied from build_l3m1_lessons_16_17.py (same chrome as the laptop's shared composer
...
composed JPG placed full-bleed on a 13.333"x7.5" slide. Level 4 specifics:
  ...
  * page totals follow the brief per lesson: L1 = 20 slides, L2 = 18, L3 = 18;
...
Usage (laptop, from docs/lesson-references):
    python build_l4m1_lessons_01_03.py --lessons 1 2 3
    python build_l4m1_lessons_01_03.py --lessons 1 --stamp-debug --no-copies   (tuning pass)
"""
...
LESSONS = {
    # Boxes are planned from the art-prompt safe zones (art-prompts.json "stamp_zones"); re-measure on the final art before the real build.
    # Lessons 1-2: every box measured on the REAL art (white/cream blob detection, tmp/blobs.py; fractions of width x / height y).
    1: {
        "title": f"Lesson 1 {T} Reading Short A CVC Words",
        "title_lines": [f"Lesson 1 {T}", "Reading", "Short A", "CVC Words"],   # Ed stands at x 0.45-0.57: 4 narrow pills stay left of him
        "title_bottom": 7.1,
        "title_max_right": 0.42,
        "folder": "L4M1-Lesson-1",
        "pptx": "L4M1-Lesson-1-Reading-Short-A-CVC-Words.pptx",
        "prefix": "l4m1-l01-slide-",
        "pages": [
            ("Welcome, Young Stewards!", True, (0.25, 0.12)),   # sky top-left; Ed's raised hand at x 0.43
            ("Short /a/ Sound: /a/ /a/ /a/", False, "bottom"),
            ("c - a - t \u2192 cat", False, "bottom"),
            ("An apple for\nhealth!", False, (0.80, 0.70)),   # 2 lines on the empty stone path: off Sofie, the cat and the logo
            ("m - a - t \u2192 mat", False, "bottom"),
            ("A cat on a mat.", False, "bottom"),
            ("h - a - t \u2192 hat", False, (0.66, 0.80)),   # off Sofie's shoes
            ("b - a - g \u2192 bag", False, (0.75, 0.80)),   # off Ed's shoes
            ("Cat | Mat | Hat | Bag", False, (0.79, 0.76)),       # board fills x 0.03-0.57 down to y 0.97
            ("A cat sat on a mat.", False, "bottom"),
            ("We use cloth bags!", False, "bottom"),
            ("God made animals and nature.", False, "bottom"),
            ("Touch the word CAT!", False, "top"),                 # crate panels sit low (y 0.60-0.80)
            ("Put the sentence in order!", False, "bottom"),
            ("Choose healthy snacks!", False, "bottom"),
            ("Show me a hat or a bag!", False, "bottom"),
            ("Trace: c - a - t", False, "bottom"),
            ("Red Apple. Cat. Mat. Hat. Bag.", False, "top"),      # label strips at y 0.73-0.81
            ("Eco-Drop Earned!", False, "bottom"),
            ("Goodbye! See you next time!", False, "bottom"),
        ],
        "stamps": {
            2: [{"kind": "word", "text": "A", "box": (0.22, 0.20, 0.42, 0.62), "color": RED}],          # brightest white glow x 0.25-0.40 (no disc drawn)
            9: [{"kind": "word", "text": w, "box": b, "group": "labels"} for w, b in zip(["Cat", "Mat", "Hat", "Bag"], [
                (0.115, 0.408, 0.285, 0.456),     # strip x 0.092-0.309, y 0.401-0.463
                (0.360, 0.409, 0.517, 0.452),     # strip x 0.340-0.537, y 0.403-0.458
                (0.120, 0.800, 0.288, 0.864),     # strip x 0.096-0.312, y 0.789-0.875
                (0.365, 0.775, 0.517, 0.832)])],  # strip x 0.343-0.539, y 0.764-0.843
            13: _boxes(["CAT", "DOG", "SUN"], [0.2595, 0.503, 0.7445], 0.635, 0.76, 0.05),              # panels x .198-.321/.440-.566/.684-.805, y .601-.796
            14: _boxes(["sat", "A cat", "on a mat."], [0.295, 0.50, 0.715], 0.515, 0.615, 0.066),       # tile faces x .215-.374/.427-.572/.637-.793, y .49-.64
            15: [{"kind": "x", "box": (0.688, 0.39, 0.79, 0.54)}],                                       # candy bag x 0.675-0.802, y 0.372-0.553
            17: [{"kind": "dotted", "text": "cat", "box": (0.25, 0.30, 0.52, 0.56), "size": 140, "baseline": 0.497}],   # lines y .366/.431/.497
            18: [{"kind": "word", "text": w, "box": (x0 + 0.012, 0.738, x1 - 0.012, 0.797), "group": "strips"}
                 for w, (x0, x1) in zip(["Apple", "Cat", "Mat", "Hat", "Bag"],
                                        [(0.075, 0.218), (0.253, 0.393), (0.426, 0.568), (0.600, 0.743), (0.777, 0.921)])],   # strips y .728-.806
        },
    },
    2: {
        "title": f"Lesson 2 {T} Pronoun I & Self-Identity",
        "title_lines": [f"Lesson 2 {T}", "Pronoun I &", "Self-Identity"],
        "title_bottom": 7.1,
        "title_max_right": 0.48,  # Sofie's legs start at x 0.52
        "folder": "L4M1-Lesson-2",
        "pptx": "L4M1-Lesson-2-Pronoun-I-and-Self-Identity.pptx",
        "prefix": "l4m1-l02-slide-",
        "pages": [
            ("Who are you?", True, (0.22, 0.12)),                  # easels top-left; Sofie's head at x 0.52-0.60
            ("Pronoun: I", False, "bottom"),
            ("I am Ed.", False, (0.68, 0.80)),   # off Ed's shoes
            ("I am Sofie.", False, (0.70, 0.80)),   # off Sofie's shoes
            ("I am ____.", False, "bottom"),
            ("Boy | Girl", False, "bottom"),
            (None, False, "bottom"),                     # I am a student. is stamped in Ed's speech bubble
            ("I can clean.", False, "bottom"),
            ("I can care for plants.", False, "bottom"),
            ("God made me special.", False, "bottom"),
            ("Look in the mirror: I am special!", False, "bottom"),
            ("Match: I am Ed / I am Sofie", False, "bottom"),
            ("I am happy.", False, "bottom"),
            ("I use gentle hands.", False, "bottom"),
            ("Speak clearly: I am ____!", False, (0.72, 0.12)),   # Remo fills the lower middle
            ("I am special. I can help.", False, "bottom"),
            ("Eco-Drop Earned!", False, "bottom"),
            ("Goodbye, Young Steward!", False, (0.34, 0.91)),   # 12:10 art: on the rug below the shoes (feet end y 0.905), left of the footer
        ],
        "stamps": {
            1: [{"kind": "word", "text": "Ed", "box": (0.71, 0.37, 0.762, 0.428)}],                      # name tag x 0.698-0.773, y 0.357-0.440
            2: [{"kind": "word", "text": "I", "box": (0.26, 0.31, 0.39, 0.63)}],                         # golden disc x 0.192-0.457, y 0.229-0.711
            5: [{"kind": "word", "text": "YOU!", "box": (0.46, 0.126, 0.537, 0.176)}],                   # plaque x 0.449-0.548, y 0.119-0.182
            6: [{"kind": "word", "text": "Boy", "box": (0.27, 0.10, 0.43, 0.185), "group": "banners"},   # banner x .242-.456, y .054-.199 (below the lesson pill)
                {"kind": "word", "text": "Girl", "box": (0.56, 0.10, 0.72, 0.185), "group": "banners"}], # banner x .534-.749, y .060-.199
            7: [{"kind": "word", "text": "I am a\nstudent.", "box": (0.45, 0.07, 0.73, 0.32)}],          # bubble x 0.381-0.797, y 0.025-0.458 (tail low-left)
            12: [{"kind": "word", "text": "I am\nSofie", "box": (0.525, 0.16, 0.715, 0.375), "group": "match"},   # block x .492-.748, y .100-.436
                 {"kind": "word", "text": "I am\nEd", "box": (0.535, 0.535, 0.725, 0.75), "group": "match"}],     # block x .498-.762, y .469-.814
            16: _boxes(["I am", "I can", "God\nmade me"], [0.395, 0.59, 0.805], 0.27, 0.45, 0.066),     # cards x .314-.476/.502-.677/.71-.90, y .22-.49
            18: [{"kind": "word", "text": "Great Job!", "box": (0.25, 0.10, 0.71, 0.285)}],                 # 12:10 art: wall banner x 0.125-0.835, y 0.05-0.33; centre x 0.48
        },
    },
    3: {
        "title": f"Lesson 3 {T} Healthy Eating: Bright Red Fruits",
        "title_lines": [f"Lesson 3 {T}", "Healthy Eating:", "Bright Red", "Fruits"],   # 4 pills keep 52pt+ inside the left half
        "title_bottom": 7.05,
        "title_max_right": 0.56,
        "folder": "L4M1-Lesson-3",
        "pptx": "L4M1-Lesson-3-Healthy-Eating-Bright-Red-Fruits.pptx",
        "prefix": "l4m1-l03-slide-",
        "pages": [
            ("Healthy Red Fruits!", True, (0.24, 0.12)),   # open sky upper-left; Ed and Sofie at x 0.58-0.86
            ("Apple", False, "bottom"),
            ("Strawberry", False, "bottom"),
            ("Cherry / Cherries", False, "bottom"),
            ("I eat red apples.", False, "bottom"),
            ("I eat strawberries.", False, "bottom"),
            ("I eat red apples and strawberries.", False, "bottom"),
            ("Red fruits make me strong!", False, "bottom"),
            ("Healthy choice: Fruit!", False, "bottom"),
            ("Seeds grow into trees!", False, (0.73, 0.80)),   # keep the seed pot clear
            ("Thank You, God, for healthy food!", False, "bottom"),
            ("Touch the red fruits!", False, "bottom"),
            ("Put red fruits in the basket!", False, (0.30, 0.83)),   # below the fruit row
            ("Chop! Mix! Eat!", False, (0.15, 0.82)),   # keep Remo whole body clear
            ("Do you have a red\nfruit at home?", False, (0.80, 0.56)),   # on the crates, off Ed and Sofie
            ("Red Apple. Red Strawberry. Red Cherry.", False, "top"),   # label cards sit low (y 0.72-0.85)
            ("Eco-Drop Earned!", False, "bottom"),
            ("Eat healthy today! Goodbye!", False, "top"),   # characters fill the lower band
        ],
        "stamps": {
            # measured on the real art (blob detection)
            9: [{"kind": "x", "box": (0.60, 0.40, 0.77, 0.80)}],                                         # chips x .588-.773 y .358-.508 + cone down to y .84
            13: [{"kind": "word", "text": "Healthy\nBasket", "box": (0.662, 0.575, 0.792, 0.745)}],       # tag x .644-.809, y .514-.764 (string hole at y .54)
            16: _boxes(["Apple", "Strawberry", "Cherry"], [0.2095, 0.4995, 0.7925], 0.745, 0.828, 0.068),   # cards x .132-.287/.422-.577/.713-.872, y .724-.849
        },
    },
}
