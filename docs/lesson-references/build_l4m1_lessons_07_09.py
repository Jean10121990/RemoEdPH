r"""Build L4M1 Young Stewards (Month 1 - The Map of My Heart) Lessons 7-9 as RemoEd PPTX decks.

Engine copied from build_l4m1_lessons_04_06.py (from build_l4m1_lessons_01_03.py) (itself from build_l3m1_lessons_16_17.py) (same chrome as the laptop's shared composer
...
composed JPG placed full-bleed on a 13.333"x7.5" slide. Level 4 specifics:
  ...
  * page totals follow the brief per lesson: L7 = 18 slides, L8 = 18, L9 = 18;
...
Usage (laptop, from docs/lesson-references):
    python build_l4m1_lessons_07_09.py --lessons 7 8 9
    python build_l4m1_lessons_07_09.py --lessons 7 --stamp-debug --no-copies   (tuning pass)
"""
...
LESSONS = {
    # Stamp boxes measured on the FINAL art (flood-fill seeds + edge profiles on the 1280x720 PNGs), 2026-09-26.
    7: {
        "title": f"Lesson 7 {T} What is a Map?",
        "title_lines": [f"Lesson 7 {T}", "What is a Map?"],
        "title_bottom": 7.05,
        "title_max_right": 0.54,
        "folder": "L4M1-Lesson-7",
        "pptx": "L4M1-Lesson-7-What-is-a-Map.pptx",
        "prefix": "l4m1-l07-slide-",
        "pages": [
            ("What is a Map?", True, (0.24, 0.12)),
            ("A map is a picture from above.", False, "bottom"),
            ("A map shows us the way.", False, (0.26, 0.115)),     # sky left of the board, above Remo's leaves
            ("Path", False, "bottom"),
            ("Find", False, (0.74, 0.70)),
            ("Treasure Map", False, "bottom"),
            ("Park Map", False, "bottom"),
            ("God guides my steps.", False, (0.66, 0.10)),
            ("Walk on the path\nto keep flowers safe!", False, (0.73, 0.56)),
            ("Finger Trace Challenge!", False, (0.63, 0.785)),
            ("Symbols match real things!", False, "bottom"),
            ("I can look at a map.", False, (0.21, 0.80)),
            ("Top | Bottom", False, (0.5, 0.838)),
            ("Touch the playground on the map!", False, "bottom"),
            ("Draw a path\non paper!", False, (0.19, 0.72)),
            ("A map shows\nus the way.", False, (0.52, 0.095)),   # above the map, between Ed's and Sofie's heads; Remo stays clear
            ("Explorer Badge & Eco-Drop!", False, "bottom"),
            ("Goodbye, Young Explorer!", False, "top"),        # goodbye group: keep Remo's whole body clear
        ],
        "stamps": {
            6: [{"kind": "x", "box": (0.727, 0.500, 0.803, 0.635)}],                                      # plain sand patch (dashed square) 70-83% x 49-64%, below-right of the chest
            13: [{"kind": "word", "text": "Top", "box": (0.410, 0.156, 0.597, 0.202), "group": "tb"},      # top frame strip 39.4-61.3% x 14.9-20.8%
                 {"kind": "word", "text": "Bottom", "box": (0.410, 0.746, 0.597, 0.796), "group": "tb"}],  # bottom frame strip 39.4-61.3% x 73.8-80.3%
        },
    },
    8: {
        "title": f"Lesson 8 {T} Mapping My Room & Home",
        "title_lines": [f"Lesson 8 {T}", "Mapping My", "Room & Home"],
        "title_bottom": 7.05,
        "title_max_right": 0.54,
        "folder": "L4M1-Lesson-8",
        "pptx": "L4M1-Lesson-8-Mapping-My-Room-and-Home.pptx",
        "prefix": "l4m1-l08-slide-",
        "pages": [
            ("Mapping My Bedroom!", True, (0.24, 0.12)),
            ("Look from the ceiling!", False, "bottom"),
            ("This is a map\nof my bedroom.", False, (0.20, 0.74)),
            ("Bed", False, "bottom"),
            ("Door | Window", False, "bottom"),
            ("Table", False, "bottom"),
            ("My bed is near the window.", False, "bottom"),
            ("Next to", False, (0.60, 0.78)),
            ("A clean room is\na happy room!", False, (0.16, 0.66)),
            ("Thank You, God, for my safe room.", False, "bottom"),
            ("Where is the door on this map?", False, "bottom"),
            ("Build the sentence!", False, "bottom"),
            ("What is near your bed?", False, (0.66, 0.78)),
            ("Draw your bed box!", False, "bottom"),
            ("Pull the\nblanket up!", False, (0.17, 0.72)),
            ("This is a map of my bedroom.", False, "bottom"),
            ("Home Steward Badge & Eco-Drop!", False, "bottom"),
            ("Goodbye! Keep your room clean!", False, (0.55, 0.098)),
        ],
        "stamps": {
            11: [{"kind": "badge", "text": t, "center": (c, 0.440), "r": 0.033}                           # badges sit ON the red dots (r 2.4% W, centred 30.8/50.1/69.2% x, 44% y),
                 for t, c in zip(["A", "B", "C"], [0.308, 0.501, 0.692])],                                  # between the white pin beads (28% y) and the symbols
            12: _boxes(["a map", "of my\nroom.", "This is"], [0.231, 0.503, 0.772], 0.555, 0.765, 0.078, "blocks"),   # block fronts 13.4-32.7 / 40.5-60 / 67.4-86.9% x, 48.6-83% y
            16: [{"kind": "word", "text": "Window", "box": (0.452, 0.222, 0.565, 0.282), "group": "labels"},   # strip 44.5-57.1% x 21.2-29.2% y (under the window)
                 {"kind": "word", "text": "Bed", "box": (0.256, 0.404, 0.356, 0.464), "group": "labels"},      # strip 24.9-36.3% x 39.4-47.4% y (on the bed)
                 {"kind": "word", "text": "Table", "box": (0.646, 0.484, 0.750, 0.547), "group": "labels"},    # strip 63.9-75.7% x 47.4-55.7% y (above the table box)
                 {"kind": "word", "text": "Door", "box": (0.339, 0.652, 0.440, 0.716), "group": "labels"}],    # strip 33.2-44.7% x 64.2-72.6% y (beside the door arc)
        },
    },
    9: {
        "title": f"Lesson 9 {T} Healthy Eating: Crunchy Green Veggies",
        "title_lines": [f"Lesson 9 {T}", "Healthy Eating:", "Crunchy Green", "Veggies"],
        "title_bottom": 7.05,
        "title_max_right": 0.54,
        "folder": "L4M1-Lesson-9",
        "pptx": "L4M1-Lesson-9-Healthy-Eating-Crunchy-Green-Veggies.pptx",
        "prefix": "l4m1-l09-slide-",
        "pages": [
            ("Crunchy Green Veggies!", True, (0.24, 0.12)),
            ("Broccoli", False, "bottom"),
            ("Spinach", False, "bottom"),
            ("Cucumber", False, "bottom"),
            ("I eat green\nbroccoli.", False, (0.17, 0.22)),
            ("I eat spinach.", False, "bottom"),
            ("I eat green broccoli and spinach.", False, "bottom"),
            ("Veggies give me power!", False, "bottom"),
            ("We grow veggies in our garden!", False, (0.32, 0.865)),
            ("God made green plants for us to eat.", False, "bottom"),
            ("Touch the healthy green veggies!", False, "bottom"),
            ("Listen to the crunch!", False, (0.66, 0.785)),
            ("Match picture to sentence!", False, "bottom"),
            ("I will try green\nveggies today!", False, (0.80, 0.60)),
            ("Trace: G - R - E - E - N", False, "bottom"),
            ("Broccoli. Spinach. Cucumber. Green!", False, "top"),
            ("Green Hero Badge & Eco-Drop!", False, "top"),
            ("Eat green today! Goodbye!", False, (0.55, 0.098)),
        ],
        "stamps": {
            12: [{"kind": "word", "text": "CRUNCH!", "box": (0.490, 0.255, 0.752, 0.445), "color": ORANGE}],     # starburst 43-81% x 11-60% (body ~47-77% x 20-50%)
            13: [{"kind": "word", "text": "I eat spinach.", "box": (0.402, 0.328, 0.650, 0.418), "group": "match"},     # TOP strip 38.8-66.3% x 31-43.5% (broccoli picture)
                 {"kind": "word", "text": "I eat green broccoli.", "box": (0.409, 0.588, 0.662, 0.685), "group": "match"}],   # BOTTOM strip 39.5-67.5% x 57.1-70.1% (spinach)
            15: [{"kind": "dotted", "text": "GREEN", "box": (0.200, 0.330, 0.640, 0.540), "size": 196, "baseline": 0.5083}],   # lines 20-64% x: top 37.6%, dashed 44.2%, base 50.8%
            16: _boxes(["Broccoli", "Spinach", "Cucumber"], [0.1745, 0.497, 0.813], 0.700, 0.786, 0.066, "cards"),   # tent cards 9.5-25.4 / 41.6-57.8 / 73.1-89.5% x, 68.3-80.3% y
            17: [{"kind": "word", "text": "Green\nHero", "box": (0.700, 0.668, 0.850, 0.822), "color": WHITE, "halo": (38, 84, 24)}],   # green disc top face 63-92% x 64-87% y
        },
    },
}
