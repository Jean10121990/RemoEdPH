r"""Build L4M1 Young Stewards (Month 1 - The Map of My Heart) Lessons 4-6 as RemoEd PPTX decks.

Engine copied from build_l4m1_lessons_01_03.py (itself from build_l3m1_lessons_16_17.py) (same chrome as the laptop's shared composer
...
composed JPG placed full-bleed on a 13.333"x7.5" slide. Level 4 specifics:
  ...
  * page totals follow the brief per lesson: L4 = 18 slides, L5 = 18, L6 = 20;
...
Usage (laptop, from docs/lesson-references):
    python build_l4m1_lessons_04_06.py --lessons 4 5 6
    python build_l4m1_lessons_04_06.py --lessons 4 --stamp-debug --no-copies   (tuning pass)
"""
...
LESSONS = {
    # Boxes below are PLANNED from the art-prompt safe zones (art-prompts.json "stamp_zones").
    # Re-measure every box on the final art (tmp/blobs.py) before the real build.
    4: {
        "title": f"Lesson 4 {T} Special Value as God's Creation",
        "title_lines": [f"Lesson 4 {T}", "Special Value", "as God's Creation"],
        "title_bottom": 7.05,
        "title_max_right": 0.54,
        "folder": "L4M1-Lesson-4",
        "pptx": "L4M1-Lesson-4-Special-Value-as-Gods-Creation.pptx",
        "prefix": "l4m1-l04-slide-",
        "pages": [
            ("You are Special!", True, (0.24, 0.12)),
            ("God made nature.", False, "bottom"),
            ("God made ME.", False, "bottom"),
            ("Purpose = A special job", False, "bottom"),
            ("God made me\nwith purpose.", False, (0.80, 0.60)),   # right-side river scenery, off Sofie's heart/hands
            ("No one else is like you!", False, "bottom"),
            ("I am loved.", False, "bottom"),
            ("My purpose is to\nlove and help.", False, (0.17, 0.60)),   # left fence gap, clear of Ed's hands and the books
            ("Care for God's world.", False, "bottom"),
            ("Touch the true words!", False, "bottom"),
            (None, False, "bottom"),                       # I am special. / God made me with purpose. stamped on the board
            ("Smile! Take a deep breath!", False, "bottom"),
            ("Say kind words to your family!", False, "bottom"),
            ("Match God's creation!", False, "bottom"),
            ("Trace: L - O - V - E", False, "bottom"),
            ("God made me with purpose.", False, "bottom"),
            ("Heart Badge &\nEco-Drop Earned!", False, (0.20, 0.72)),   # soil at left, clear of the heart
            ("You are a blessing! Goodbye!", False, "top"),   # sky between the waving hands; keeps Remo's whole body clear
        ],
        "stamps": {   # measured on the real art (flood-fill of each blank surface, 1280x720 PNGs), 2026-09-26
            10: _boxes(["SPECIAL", "LOVED", "PURPOSE"], [0.2745, 0.499, 0.7195], 0.272, 0.338, 0.068, "stars"),   # gold stars 20-35 / 42.5-57 / 64.5-79% x 17-42%; white hub y 26-35%
            11: [{"kind": "word", "text": "I am special.", "box": (0.29, 0.43, 0.65, 0.56), "group": "board"},   # cream board face 25-69% x 38-81%
                 {"kind": "word", "text": "God made me\nwith purpose.", "box": (0.29, 0.60, 0.65, 0.77), "group": "board"}],
            13: [{"kind": "word", "text": "You are\nkind!", "box": (0.53, 0.08, 0.80, 0.30)}],                  # bubble 47-85% x 4-34%
            14: [{"kind": "word", "text": "God\nmade me", "box": (0.52, 0.25, 0.725, 0.44), "group": "match"},     # TOP block (sun picture) 49.5-75% x 22-47%
                 {"kind": "word", "text": "God made\nthe sun", "box": (0.52, 0.575, 0.725, 0.765), "group": "match"}],   # BOTTOM block (child) 49.5-75% x 55-79%
            15: [{"kind": "dotted", "text": "LOVE", "box": (0.345, 0.35, 0.665, 0.60), "size": 212, "baseline": 0.555}],   # guide lines 40.5 / 47.5 / 55.5% down, 35-66% x
            16: [{"kind": "word", "text": "GOD\nMADE ME", "box": (0.31, 0.375, 0.433, 0.497), "group": "signs"},      # signs 30-44.5% x 35-52% / 54-68% x 37.5-54%
                 {"kind": "word", "text": "I HAVE\nPURPOSE", "box": (0.554, 0.40, 0.672, 0.515), "group": "signs"}],
        },
    },
    5: {
        "title": f"Lesson 5 {T} Honoring Every Unique Person",
        "title_lines": [f"Lesson 5 {T}", "Honoring Every", "Unique Person"],
        "title_bottom": 7.05,
        "title_max_right": 0.54,
        "folder": "L4M1-Lesson-5",
        "pptx": "L4M1-Lesson-5-Honoring-Every-Unique-Person.pptx",
        "prefix": "l4m1-l05-slide-",
        "pages": [
            ("Loving Everyone!", True, (0.24, 0.12)),
            ("Different flowers, one garden!", False, "bottom"),
            ("We are different.", False, "bottom"),
            ("I treat everyone with love.", False, "bottom"),
            ("Use kind words!", False, "bottom"),
            ("Use gentle hands.", False, "bottom"),
            ("Stop, I don't like that.", False, "bottom"),
            ("Love your neighbor.", False, "bottom"),
            ("How do we show love?", False, "bottom"),
            ("Say: Come play with us!", False, "bottom"),
            (None, False, "bottom"),                       # I use kind words. / I treat everyone with love. stamped on the cards
            ("Treat animals with love too!", False, "bottom"),
            ("Kindness Pledge: I promise to be kind!", False, "bottom"),
            ("Sort: Helping a friend \u2192 Kind!", False, "top"),   # keeps the Kind / Unkind strips visible
            ("Say something kind to Teacher!", False, "top"),   # keeps Remo's whole body clear
            ("I treat everyone with love.", False, "top"),   # above the heads; keeps Remo clear
            ("Unity Badge & Eco-Drop!", False, "bottom"),
            ("Be kind today! Goodbye!", False, "top"),
        ],
        "stamps": {
            9: [{"kind": "badge", "text": "A", "center": (0.075, 0.17), "r": 0.03},       # plain sky top-left of each photo panel
                {"kind": "badge", "text": "B", "center": (0.51, 0.17), "r": 0.03}],
            11: [{"kind": "word", "text": "I use\nkind words.", "box": (0.39, 0.27, 0.635, 0.45), "group": "cards"},        # cream cards 37-65% x 24-48 / 53-78%
                 {"kind": "word", "text": "I treat everyone\nwith love.", "box": (0.39, 0.565, 0.635, 0.745), "group": "cards"}],
            14: [{"kind": "word", "text": "Kind", "box": (0.29, 0.792, 0.445, 0.852), "group": "boxes"},   # strips 27-46 / 56-74% x 78-86%
                 {"kind": "word", "text": "Unkind", "box": (0.575, 0.792, 0.727, 0.852), "group": "boxes"}],
        },
    },
    6: {
        "title": f"Lesson 6 {T} Reading Short E CVC Words",
        "title_lines": [f"Lesson 6 {T}", "Reading Short E", "CVC Words"],
        "title_bottom": 7.05,
        "title_max_right": 0.54,
        "folder": "L4M1-Lesson-6",
        "pptx": "L4M1-Lesson-6-Reading-Short-E-CVC-Words.pptx",
        "prefix": "l4m1-l06-slide-",
        "pages": [
            ("Short E CVC Words!", True, (0.24, 0.12)),
            ("Short /e/ Sound: /e/ /e/ /e/", False, (0.32, 0.807)),   # slid left off Remo
            ("h - e - n \u2192 hen", False, "bottom"),
            ("r - e - d \u2192 red", False, "bottom"),
            ("n - e - t \u2192 net", False, "bottom"),
            ("The red hen is in the net.", False, "bottom"),
            ("p - e - n \u2192 pen", False, "bottom"),
            ("b - e - d \u2192 bed", False, "bottom"),
            ("I keep my room neat!", False, "bottom"),
            ("Hen | Red | Net | Pen | Bed", False, "bottom"),
            ("A red pen on the bed.", False, "bottom"),
            ("Sleep early for a healthy body!", False, "bottom"),
            ("Thank You, God, for my home and bed.", False, "bottom"),
            ("Which word is H - E - N?", False, (0.42, 0.835)),   # below the word boxes
            ("Fill in the blank: r _ d", False, (0.36, 0.855)),   # table front, below the tiles
            ("Show me a pen!", False, "bottom"),
            ("Trace: p - e - n", False, "bottom"),
            ("Short /e/ words review!", False, "bottom"),
            ("Eco-Drop Earned!", False, "bottom"),
            ("Goodbye, Young Steward!", False, (0.575, 0.095)),   # night sky between Ed's head and the moon
        ],
        "stamps": {
            2: [{"kind": "word", "text": "E", "box": (0.245, 0.355, 0.365, 0.575), "color": ORANGE}],                 # glowing disc 18.7-42.4% x 25-67.6%
            10: [{"kind": "word", "text": w, "box": (x0 + 0.012, 0.594, x1 - 0.012, 0.632), "group": "strips"}          # white band at the foot of each card, 59-63.5% down
                 for w, (x0, x1) in zip(["Hen", "Red", "Net", "Pen", "Bed"], [(0.192, 0.299), (0.323, 0.428), (0.452, 0.555), (0.578, 0.680), (0.705, 0.809)])],
            12: [{"kind": "word", "text": "8:00 PM", "box": (0.675, 0.33, 0.80, 0.42), "color": CLOCK_GREEN, "halo": (10, 40, 20)}],  # black screen 65.6-82% x 31-44%
            14: _boxes(["HEN", "HAT", "HOT"], [0.249, 0.500, 0.747], 0.648, 0.772, 0.078, "boxes"),              # cream boxes 15-35 / 40-60 / 65-84.5% x 62-80%
            15: [{"kind": "word", "text": "r _ d", "box": (0.34, 0.20, 0.68, 0.41)},                             # panel 30-72% x 16-44%
                 *[dict(b, size=96) for b in _boxes(["a", "e", "o"], [0.281, 0.507, 0.730], 0.695, 0.755, 0.07, "tiles")]],                   # round tile tops 70.8-76.6% down
            17: [{"kind": "dotted", "text": "pen", "box": (0.35, 0.36, 0.735, 0.66), "size": 215, "baseline": 0.595}],   # guide lines 38.5 / 49 / 59.5% down, 35-73.5% x
            18: [{"kind": "word", "text": w, "box": (c - 0.036, 0.453, c + 0.036, 0.486), "group": "strips"}            # blank bars inside each frame, 44.7-49% down
                 for w, c in zip(["Hen", "Net", "Red", "Pen", "Bed"], [0.184, 0.343, 0.503, 0.661, 0.820])],
        },
    },
}
