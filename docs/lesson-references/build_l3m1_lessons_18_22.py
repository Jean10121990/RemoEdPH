...
# Stamp boxes, title breaks and chip positions for Lessons 18-22 are measured on the final art.
...
LESSONS = {
    ...
    21: {
        ...
        "title_lines": ["Lesson 21 \u2013", "App Garden", "Milestone 1"],
        "title_max_right": 0.53,   # final art: Ed / Sofie / Remo fill x 0.55-0.92 on slide 1; lower-left is grass
        "folder": "L3M1-Lesson-21",
        "pptx": "L3M1-Lesson-21-App-Garden-Milestone-1.pptx",
        "prefix": "l3m1-l21-slide-",
        "pages": [
            ("Welcome to Garden Milestone 1!", True, (0.285, 0.29)),   # left sky: Ed's raised hand fills the top-middle
        ...
        "stamps": {
            # four white cards on the bench, y 0.43-0.76 (measured)
            14: [{"kind": "word", "text": w, "box": (a + 0.015, 0.53, b - 0.015, 0.65), "group": "row"} for w, (a, b) in
                 zip(["Garden", "Grow", "Plant", "Water"], [(0.20, 0.34), (0.36, 0.495), (0.515, 0.65), (0.67, 0.81)])],
        },
    },
    22: {
        "title": "Lesson 22 \u2013 Month 1 Grand Showcase",
        "title_lines": ["Lesson 22 \u2013", "Month 1", "Grand", "Showcase"],   # 4 short pills: Grand Showcase in one pill would touch Ed
        "title_bottom": 7.1,        # low on the empty stage floor; top pill clears the left banner
        "title_max_right": 0.42,    # final art: Ed's raised hand / jacket / shoes start at x 0.465 on slide 1
        # slide 5: camera-frame corner brackets sit under the page chrome; a 1.12 crop pushes the lower pair off-frame
        # and tucks the upper pair under the lesson label / page badge
        "art_zoom": {5: (1.12, 0.5, 0.1)},
        ...
        "pages": [
            ...
            ("Show Your Written Name!", False, (0.5, 0.825)),   # just below the (zoomed) paper
            ...
            ("My name is ____, and I can read!", False, "bottom"),   # fill-in blank (4 underscores), per Kristine
            ...
            ("Goodbye & See You in Month 2!", False, (0.5, 0.10)),   # sky: a bottom chip would touch Remo's face
        ],
        "stamps": {
            # measured on final art
            3: [{"kind": "dotted", "text": "Name", "box": (0.34, 0.26, 0.66, 0.58)}],       # whiteboard face x 0.30-0.70, y 0.19-0.645
            5: [{"kind": "word", "text": "Ed", "box": (0.45, 0.60, 0.572, 0.745)},        # paper after the 1.12 crop: x 0.41-0.61, y 0.55-0.79
                # ends of the top brackets peeking out beside the lesson label / page badge -> plain curtain from just below
                {"kind": "clone", "box": (0.0, 0.025, 0.035, 0.09), "dy": 0.07},
                {"kind": "clone", "box": (0.962, 0.025, 1.0, 0.09), "dy": 0.07}],
            7: [{"kind": "word", "text": "The cat sat on the mat\nwith a hat.", "box": (0.355, 0.57, 0.655, 0.685)}],   # cream strip x 0.33-0.68, y 0.55-0.70
            14: [{"kind": "word", "text": w, "box": (0.35, c - 0.045, 0.76, c + 0.045), "align": "left", "group": "rows"}   # right of the checks
                 for w, c in zip(["Name", "Phonics", "Tech Stewardship", "Values", "Home Nouns"], [0.19, 0.31, 0.42, 0.535, 0.65])],
        },
    },
}
...
def compose_slide(src, dest, title, page, total, phrase, is_title, chip_pos, stamps, title_bottom=6.42, title_lines=None,
                  title_max_right=None, zoom=None):
    ...
    for i, (phrase, is_title, pos) in enumerate(L["pages"], 1):
        ...
        if only is None or i in only:
            compose_slide(art_dir / f"{L['prefix']}{i:02d}.png", dest, L["title"], i, total, phrase, is_title, pos, stamps.get(i, []),
                          ...
                          L.get("title_max_right"), L.get("art_zoom", {}).get(i))
